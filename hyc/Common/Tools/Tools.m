...
+ (double)getDoubleWithString:(NSString *)st
{
    ...
    // 判断最后一位是不是最高位
    NSString *lastSt = [stArr lastObject];
    double lastUnit = 0;
    
    if ((stArr.count>=3 && stArr.count%2 != 0) && ([lastSt is:@"万"] || [lastSt is:@"亿"])) {
        lastUnit = [self getNumWithString:lastSt];
        [stArr removeLastObject];
    }
    
    
    // 一万   十万    二万五千三百一十五    一万五   二百五十万
